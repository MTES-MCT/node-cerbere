...
/**
 * Authenticate with the Cerbere (CAS 2.0 / SAML 1.1) server.
 * `callback` is called with (err, auth_status, username, extended)
 *
 * @param {String} req
 *     A HTTP express request
 * @param {String} res
 *     A HTTP express response
 * @param {Function} callback
 *     callback(err, auth_status, username, extended).
 *     `extended` is an object containing:
 *       - username
 ...
 * @api public
 */
Cerbere.prototype.authenticate = function(req, res, callback, service) {

  var reqURL = url.parse(req.url, true);    
  // Try to extract the Cerbere (CAS 2.0) ticket from the URL
  var ticket = reqURL.query['ticket'];
  // No ticket, so we haven't been sent to the Cerbere server yet
  if (!ticket) {
    // redirect to Cerbere server now
    var redirectURL = this.cerbere_url + '/login?service=' + encodeURIComponent(service);
...
// We have a ticket! 
    ...
    // Validate it with the Cerbere server now
    // CAS 2.0 with SAML 1.1 to get attributes
